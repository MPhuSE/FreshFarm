    ...
    class CartService
    {
        public function getCartDetails(int $userId): array
        {
            ...
            foreach ($items as $item) {
                ...
                $availableStock = $product->inventory ? (float) $product->available_quantity : 0;
        ...
        public function addItem(int $userId, int $productId, float $quantity): array
        {
            ...
            if (! $product) {
                throw new ApiException('Sản phẩm không tồn tại', 'PRODUCT_NOT_FOUND', 404);
            }

            $availableStock = $product->inventory ? (float) $product->available_quantity : 0;

            if ($availableStock < $quantity) {
                throw new ApiException('Số lượng sản phẩm vượt quá tồn kho', 'INSUFFICIENT_STOCK', 409);
            }
        ...
        public function updateItem(int $userId, int $cartItemId, float $quantity): array
        {
            ...
            if (! $cart) {
                throw new ApiException('Giỏ hàng không tồn tại', 'CART_NOT_FOUND', 404);
            }
            ...
            if (! $cartItem) {
                throw new ApiException('Sản phẩm trong giỏ hàng không tồn tại', 'CART_ITEM_NOT_FOUND', 404);
            }

            $product = Product::with('inventory')->find($cartItem->product_id);
            $availableStock = $product && $product->inventory ? (float) $product->available_quantity : 0;
